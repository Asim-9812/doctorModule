


List<Map<String, dynamic>> doctorInfo = [
  {
    'name': 'Dr. John Doe',
    'specialized field': 'Cardiology',
    'contact': '+1 123-456-7890',
    'email': 'johndoe@example.com',
  },
  {
    'name': 'Dr. Jane Smith',
    'specialized field': 'Dermatology',
    'contact': '+1 987-654-3210',
    'email': 'janesmith@example.com',
  },
  {
    'name': 'Dr. Michael Johnson',
    'specialized field': 'Orthopedics',
    'contact': '+1 555-123-4567',
    'email': 'michaeljohnson@example.com',
  },
  {
    'name': 'Dr. Emily Davis',
    'specialized field': 'Pediatrics',
    'contact': '+1 222-333-4444',
    'email': 'emilydavis@example.com',
  },
  {
    'name': 'Dr. William Brown',
    'specialized field': 'Ophthalmology',
    'contact': '+1 777-888-9999',
    'email': 'williambrown@example.com',
  },
];


List<Map<String, dynamic>> hospitalInfo = [
  {
    'name': 'ABC Hospital',
    'address': '123 Main St, City, Country',
    'contact': '+1 123-456-7890',
    'longitude': 85.33967032938445,
    'latitude': 27.685050583950968,
  },
  {
    'name': 'XYZ Hospital',
    'address': '456 Elm St, City, Country',
    'contact': '+1 987-654-3210',
    'longitude': 85.34252793705424,
    'latitude': 27.686133346740386,
  },
  {
    'name': '123 Medical Center',
    'address': '789 Oak St, City, Country',
    'contact': '+1 555-123-4567',
    'longitude': 85.34191173794464,
    'latitude': 27.6861908641266,
  },
  {
    'name': 'City General Hospital',
    'address': '321 Pine St, City, Country',
    'contact': '+1 222-333-4444',
    'longitude': 85.34076463133897,
    'latitude': 27.68592022468117,
  },
  {
    'name': 'Sunshine Hospital',
    'address': '777 Maple St, City, Country',
    'contact': '+1 777-888-9999',
    'longitude': 85.34173536318949,
    'latitude': 27.685483288430467,
  },
];

List<Map<String, dynamic>> clinicInfo = [
  {
    'name': 'ABC Clinic',
    'address': '123 Main St, City, Country',
    'contact': '+1 123-456-7890',
    'longitude': 85.3406383639472,
    'latitude': 27.685579146939734,
  },
  {
    'name': 'XYZ Clinic',
    'address': '456 Elm St, City, Country',
    'contact': '+1 987-654-3210',
    'longitude': 85.34266413795143,
    'latitude': 27.68630255666499,
  },
  {
    'name': '123 Medical Clinic',
    'address': '789 Oak St, City, Country',
    'contact': '+1 555-123-4567',
    'longitude': 85.34124961643649,
    'latitude': 27.685570809956374,
  },
  {
    'name': 'City General Clinic',
    'address': '321 Pine St, City, Country',
    'contact': '+1 222-333-4444',
    'longitude': 85.34212985960297,
    'latitude': 27.68632427595156,
  },
  {
    'name': 'Sunshine Clinic',
    'address': '777 Maple St, City, Country',
    'contact': '+1 777-888-9999',
    'longitude': 85.34052313586756,
    'latitude': 27.686038091858495,
  },
];

List<Map<String, dynamic>> ambulanceInfo = [
  {
    'name': 'ABC Ambulance Services',
    'address': '123 Main St, City, Country',
    'contact': '+1 123-456-7890',
    'longitude': 85.33970933453612,
    'latitude': 27.68515241585853,
  },
  {
    'name': 'XYZ Ambulance Services',
    'address': '456 Elm St, City, Country',
    'contact': '+1 987-654-3210',
    'longitude': 85.34238033434523,
    'latitude': 27.68598445073239,
  },
  {
    'name': '123 Medical Ambulance Services',
    'address': '789 Oak St, City, Country',
    'contact': '+1 555-123-4567',
    'longitude': 85.34173873191935,
    'latitude': 27.68621598967255,
  },
];


List<Map<String, dynamic>> notificationData = [
  {
    'name': 'John Doe',
    'createdDate': '2023-07-26',
    'userImg': 'assets/icons/doctor.png',
    'notificationDescription': 'You have a new message!',
    'type': 'general',
  },
  {
    'name': 'Alice Smith',
    'createdDate': '2023-07-25',
    'userImg': null,
    'notificationDescription': 'Your order has been shipped.',
    'type': 'personal',
  },
  {
    'name': 'Bob Johnson',
    'createdDate': '2023-07-24',
    'userImg': 'assets/icons/patient.png',
    'notificationDescription': 'Today is a special event!',
    'type': 'general',
  },
  {
    'name': 'Jane Doe',
    'createdDate': '2023-07-23',
    'userImg': null,
    'notificationDescription': 'You have a new notification.',
    'type': 'personal',
  },
  {
    'name': 'Tom Smith',
    'createdDate': '2023-07-22',
    'userImg': 'assets/icons/patient_login.png',
    'notificationDescription': 'Your subscription is ending soon.',
    'type': 'general',
  },
  {
    'name': 'Tom Smith',
    'createdDate': '2023-07-22',
    'userImg': 'assets/icons/patient_login.png',
    'notificationDescription': 'Your subscription is ending soon.',
    'type': 'general',
  },
  {
    'name': 'Tom Smith',
    'createdDate': '2023-07-22',
    'userImg': 'assets/icons/patient_login.png',
    'notificationDescription': 'Your subscription is ending soon.',
    'type': 'general',
  },
  {
    'name': 'Tom Smith',
    'createdDate': '2023-07-22',
    'userImg': 'assets/icons/patient_login.png',
    'notificationDescription': 'Your subscription is ending soon.',
    'type': 'general',
  },
  {
    'name': 'Tom Smith',
    'createdDate': '2023-07-22',
    'userImg': 'assets/icons/patient_login.png',
    'notificationDescription': 'Your subscription is ending soon.',
    'type': 'general',
  },
  {
    'name': 'Tom Smith',
    'createdDate': '2023-07-22',
    'userImg': 'assets/icons/patient_login.png',
    'notificationDescription': 'Your subscription is ending soon.',
    'type': 'general',
  },
  {
    'name': 'Tom Smith',
    'createdDate': '2023-07-22',
    'userImg': 'assets/icons/patient_login.png',
    'notificationDescription': 'Your subscription is ending soon.',
    'type': 'general',
  },
  {
    'name': 'Tom Smith',
    'createdDate': '2023-07-22',
    'userImg': 'assets/icons/patient_login.png',
    'notificationDescription': 'Your subscription is ending soon.',
    'type': 'general',
  }, {
    'name': 'Tom Smith',
    'createdDate': '2023-07-22',
    'userImg': 'assets/icons/patient_login.png',
    'notificationDescription': 'Your subscription is ending soon.',
    'type': 'general',
  }, {
    'name': 'Tom Smith',
    'createdDate': '2023-07-22',
    'userImg': 'assets/icons/patient_login.png',
    'notificationDescription': 'Your subscription is ending soon.',
    'type': 'general',
  },
];

List<Map<String, dynamic>> closeCircle = [
  {
    "name": "Dr. John Doe",
    "picture": "assets/icons/doctor.png",
    "specializeIn": "Cardiology",
    "gender": "Male",
  },
  {
    "name": "Dr. Jane Smith",
    "picture": "assets/icons/doctor_login.png",
    "specializeIn": "Pediatrics",
    "gender": "Others",
  },
  {
    "name": "Dr. Robert Johnson",
    "picture": null,
    "specializeIn": "Orthopedics",
    "gender": "Male",
  },
  {
    "name": "Dr. Sarah Williams",
    "picture": "assets/icons/patient.png",
    "specializeIn": "Physiology",
    "gender": "Female",
  },
  {
    "name": "Dr. Michael Lee",
    "picture": "assets/icons/patient_login.png",
    "specializeIn": "Neurology",
    "gender": "Male",
  },
  {
    "name": "Dr. Emily Brown",
    "picture": "assets/icons/org_login.png",
    "specializeIn": "Ophthalmology",
    "gender": "Others",
  },
  {
    "name": "Dr. David Wilson",
    "picture": null,
    "specializeIn": "Dermatology",
    "gender": "Male",
  },
  {
    "name": "Dr. Olivia Davis",
    "picture": "assets/icons/hospital.png",
    "specializeIn": "Endocrinology",
    "gender": "Female",
  },
];

List<Map<String, dynamic>> patientList = [
  {
    'name': 'John Doe',
    'address': '123 Main Street',
    'contact': '+1 (555) 123-4567',
    'department': 'Cardiology',
    'picture': 'assets/icons/patient.png',
    'gender': "Male",
  },
  {
    'name': 'Jane Smith',
    'address': '456 Oak Avenue',
    'contact': '+1 (555) 987-6543',
    'department': 'Orthopedics',
    'picture': 'assets/icons/patient.png',
    'gender': "Female",
  },
  {
    'name': 'Alice Johnson',
    'address': '789 Elm Lane',
    'contact': '+1 (555) 321-7890',
    'department': 'Pediatrics',
    'picture': 'assets/icons/patient.png',
    'gender': "Female",
  },
  {
    'name': 'Bob Wilson',
    'address': '101 Pine Street',
    'contact': '+1 (555) 222-3333',
    'department': 'Neurology',
    'picture': 'assets/icons/patient.png',
    'gender': "Male",
  },
  {
    'name': 'Emily Brown',
    'address': '222 Oak Lane',
    'contact': '+1 (555) 444-5555',
    'department': 'Dermatology',
    'picture': 'assets/icons/patient.png',
    'gender': "Others",
  },
  {
    'name': 'Michael Lee',
    'address': '333 Maple Avenue',
    'contact': '+1 (555) 777-8888',
    'department': 'Gastroenterology',
    'picture': 'assets/icons/patient.png',
    'gender': "Male",
  },
  {
    'name': 'Sarah Adams',
    'address': '444 Elm Street',
    'contact': '+1 (555) 999-0000',
    'department': 'Oncology',
    'picture': 'assets/icons/patient.png',
    'gender': "Female",
  },
  {
    'name': 'David Johnson',
    'address': '555 Oak Avenue',
    'contact': '+1 (555) 111-2222',
    'department': 'ENT',
    'picture': 'assets/icons/patient.png',
    'gender': "Male",
  },
  {
    'name': 'Jennifer Miller',
    'address': '666 Maple Lane',
    'contact': '+1 (555) 333-4444',
    'department': 'Urology',
    'picture': 'assets/icons/patient.png',
    'gender': "Others",
  },
  {
    'name': 'Robert Brown',
    'address': '777 Elm Street',
    'contact': '+1 (555) 666-7777',
    'department': 'Cardiology',
    'picture': 'assets/icons/patient.png',
    'gender': "Male",
  },
];


List<Map<String, dynamic>> appointments = [
  {
    "patientName": "John Smith",
    "appointmentTitle": "Regular Check-up",
    "appointmentTime": "2023-07-27 09:30 AM",
  },
  {
    "patientName": "Jane Doe",
    "appointmentTitle": "Follow-up on Medication",
    "appointmentTime": "2023-07-27 11:15 AM",
  },
  {
    "patientName": "Robert Johnson",
    "appointmentTitle": "Pre-operation Consultation",
    "appointmentTime": "2023-07-28 02:00 PM",
  },
  {
    "patientName": "Alice Brown",
    "appointmentTitle": "Dental Cleaning",
    "appointmentTime": "2023-07-29 10:45 AM",
  },
  {
    "patientName": "Emily Williams",
    "appointmentTitle": "Eye Examination",
    "appointmentTime": "2023-07-29 03:30 PM",
  },
  {
    "patientName": "Michael Lee",
    "appointmentTitle": "Blood Test",
    "appointmentTime": "2023-07-30 09:00 AM",
  },
  {
    "patientName": "Laura Adams",
    "appointmentTitle": "Physical Therapy",
    "appointmentTime": "2023-07-30 02:30 PM",
  },
  {
    "patientName": "David Wilson",
    "appointmentTitle": "X-ray Evaluation",
    "appointmentTime": "2023-07-31 11:00 AM",
  },
  {
    "patientName": "Olivia Davis",
    "appointmentTitle": "Diabetes Consultation",
    "appointmentTime": "2023-08-01 04:15 PM",
  },
  {
    "patientName": "Sophia Martin",
    "appointmentTitle": "Allergy Test",
    "appointmentTime": "2023-08-01 09:45 AM",
  },
  // Add more appointment data here...
];


